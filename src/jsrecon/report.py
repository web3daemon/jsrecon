...
def to_markdown(a: Analysis, target: str) -> str:
    ...
    if a.endpoints:
        lines += ["## Endpoints", "", "| Method | URL | Confidence | Line |", "|---|---|---|---|"]
        for e in a.endpoints:
            lines.append(f"| {e.method or '—'} | `{e.url}` | {e.kind} | {e.line} |")
        lines.append("")
    if a.graphql:
    ...
    if a.secrets:
        lines += ["## Secrets that should not ship to the client", "",
                  "| Severity | Kind | Preview | Line |", "|---|---|---|---|"]
        for s in a.secrets:
            lines.append(f"| {s.severity} | {s.label} | `{s.preview}` | {s.line} |")
        lines.append("")
    return "\n".join(lines)
...
def to_openapi(a: Analysis, target: str) -> dict:
    ...
    for e in a.endpoints:
        ...
        paths[path][method] = {
            "summary": f"seen in client JS (line {e.line}, {e.kind})",
